...
class FilterDDriveEKF:
    ...
    def __init__(self, odometer_output, sensor_output):
        ...
        self.sensor = sensor_output  # sampel, landmarks,SensorLandmarks2d
        ...
        self.useNumericPrediction = False  # do mean prediction using ode45❗️
    ...
    def filter_step(self, t, X, control_output):
            ...
            while tNow < sensor.t[i]:
                ...
                while tNow < tNextUpdate:
                    if iPredict <= len(odometer):
                        tNext = odometer.t[iPredict-1]  # index-1
        ...
        data = {'pose': self.xs, 'cov': self.Ps, 't': self.ts, 'state': self.states}
        self.out = pd.DataFrame(data)
        return self.out

    def do_prediction(self, x, P, u, T):  # Koppelnavigation (Dead Reckoning).
        ...
        # input：Geschwindigkeit werden hier durch das Odometer bereitgestellt
        # output：aktualisierte Zustandsschätzung ̂𝒙𝒌+𝟏 sowie die zugehörige Kovarianzmatrix 𝑷̂𝒌+𝟏 zum Zeitpunkt 𝒕𝒌+𝟏
        global v, omega
        ...
        if self.useNumericPrediction:
            ...
            if np.abs(omega) < 1e-12:
                x = np.array(x) + np.array([v * T * ck,  #可能要弄成列 形式的
                                            v * T * sk,
                                            0])
            else:
                x = np.array(x) + np.array([v / omega * (skp1 - sk),
                                            v / omega * (ck - ckp1),
                                            T * omega])
        ...
        if not self.useExactDiscretization:
            ...
            while True:
                ...
                S = S + D
                i = i + 1
                if (np.abs(D) <= 2.220446049250313e-16).all():  # np.finfo(float).eps = 2.220446049250313e-16
        ...
        # durch Linearisierung der Funktion 𝒇̂dis(̂𝒙𝑘, 𝒖̃𝑘) aus Teilaufgabe (a) gewinnen?
            ...
            if np.abs(omega) < 1e-12:
                A_dis = np.array([[1, 0, -v * T * sk],
                                  [0, 1, v * T * ck],
                                  [0, 0, 1]])
                B_dis = np.array([[R_R * T / 2 * ck, R_L * T / 2 * ck],
                                  [R_R * T / 2 * sk, R_L * T / 2 * sk],
                                  [0,                0]])
            else:
                A_dis = np.array([[1, 0, v / omega * (ckp1 - ck)],
                                  [0, 1, v / omega * (skp1 - sk)],
                                  [0, 0, 1]])
                B_dis = np.array([[R_R / (2 * omega) * (v * T / a * ckp1 + R_L * dtheta_L / (a * omega) * (sk - skp1)),
    ...
    def do_update(self, x, P, meas, landmarks):
        ...
        if not self.useCartesianSensor:
            if self.useBearing:
                ...
                c4 = np.hstack((c1, c2, c3))  # Stack arrays in sequence horizontally (column wise).
                C = np.vstack((C, c4))  # Stack arrays in sequence vertically (row wise).
            ...
            if self.useRange:
                ...
                C = np.vstack((C, c4))  # C = [C; [-b(:, 1) ./ y_pred, -b(:, 2) ./ y_pred, zeros(size(visIdx))]]
            ...
            for i in range(len(visIdx)):
                ...
                # delta_y[2 * (i+1) - 1, 2 * (i+1)] = y_meas - y_pred
                c1 = np.array([[-np.cos(x[2]), -np.sin(x[2]), np.cos(x[2]) * b[i, 1] - np.sin(x[2]) * b[i, 0]],
        ...
        K = np.dot(k1, np.linalg.inv(k2))  # K = P * C' / (C * P * C' + W)
    ...
    def vomega_model(self, x, t_odeint):
        phi = x[2]
        dx = (v * np.cos(phi), v * np.sin(phi), omega)
        return dx
