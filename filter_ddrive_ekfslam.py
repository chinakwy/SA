...
class FilterDDriveEKFSlAM:  # Simultaneous Localization And Mapping with Extended Kalman Filter
    ...
    def __init__(self, odometer_output, sensor_output):
        ...
        self.rangeError = 2 / 100  # sigma of assumed range error in percent of the aquired distance value
        ...
        # varianbles for the extra visualization window
        self.states = []
        self.state_cov = []
        self.xhatlog = []
        self.yhatlog = []
    ...
    def do_prediction(self, x, P, u, T):
        ...
        if self.useNumericPrediction:
            ...
            if np.abs(omega) < 1e-12:
                x[0:3] = np.array(x)[0:3] + np.array([v * T * ck,
                                                      v * T * sk,
                                                      0])
            else:
                x[0:3] = np.array(x)[0:3] + np.array([v / omega * (skp1 - sk),
                                                      v / omega * (ck - ckp1),
                                                      T * omega])
    ...
    def vomega_model(self, x, t_odeint):
        phi = x[2]
        dx = np.array([v * np.cos(phi), v * np.sin(phi), omega])
        return dx
