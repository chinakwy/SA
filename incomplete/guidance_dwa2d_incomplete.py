...
#######################################################
    ...
    def dwa_step(self, t, relativeGoal, rangefinder_data):
        ...
        # np.hstack(), np.matlib.repmat()
        # TODO:
        obstacleLines = np.hstack((0 + np.matlib.repmat(rayExtentLeft.reshape(-1, 1) + minDistance, 1, 2) * 0,  # TODO
                                   0 - np.matlib.repmat(rayExtentRight.reshape(-1, 1) + minDistance, 1, 2) * 0))  # TODO
        ...
        # np.max(()) ,np.min(())
        # TODO
        minV = np.max((0, self.vMin))   # TODO
        maxV = np.min((0, 0))   # TODO
        minOmega = np.max((0, 0))   # TODO
        maxOmega = np.min((0, 0))   # TODO
        # Generate the candidates as two matrices
        # - velocities and
        # - omegas
        # Hint: use the 'np.meshgrid' function, np.linspace(start, stop, num)
        # TODO
        omegas, velocities = np.meshgrid(0, 0)  # TODO
        ...
        # Hint: use the 'max_mtlb' function at the bottom. np.abs()
        # TODO
        dDeceleration = max_mtlb(0, np.abs(0))   # TODO
        ...
        # np.abs(), np.remainder, np.arctan2, np.pi
        ...
        #  deceleration phase
        # Hint: use the 'max_mtlb' and 'min_mtlb' function at the bottom.
        # TODO
        utility_dist = []  # TODO
...
def max_mtlb(a, b):
    if isinstance(a, int):  # Comparison between int and arrays
        c = [[np.max((a, b[i][j])) for j in range(len(b[i]))] for i in range(len(b))]
    else:  # Comparison between arrays
