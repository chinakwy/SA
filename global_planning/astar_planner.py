...
class AstarPlanner:
    ...
    def __init__(self, map, start, goal):
        ...
        # A* per-cell data
        self.closed = np.full(map.shape, False)  # boolean matrix: true for each cell in closed list (initially, no cell is closed)
        ...
        self.parent = np.zeros(map.shape, dtype=np.uint32)  # matrix of 'predecessor' cells (valid for open/closed cells only)
        ...
        if self.start.any() < 0 or self.start[0] > self.w or self.start[1] > self.h:
            ...
            self.delta = np.abs([self.x_start - self.x_goal, self.y_start - self.y_goal])
            self.f_cost.reshape(-1)[self.startCell] = 10 * np.abs(self.delta[0] - self.delta[1]) + 14 * np.min(self.delta)
    ...
    def run(self, *args):
        ...
        if not len(self.errorMessage):
            ...
            while ~self.closed.reshape(-1)[self.goalCell][0] and self.nOpenCells > 0 and iterations > 0:
                ...
                cell = self.openHeap_popHead()
                self.closed.reshape(-1)[cell] = True
                ...
                for i in range(8):
                    if validNeighbours[i]:
                        ...
                        if heapPos > 0:
                            ...
                            g_cost_reduction = self.g_cost.reshape(-1)[neighbourCell] - \
                                               (self.g_cost.reshape(-1)[cell] + neighbourCosts[i])

                            if g_cost_reduction > 0:
                                # g_cost reduction possible, update cell
                                self.g_cost.reshape(-1)[neighbourCell] = self.g_cost.reshape(-1)[neighbourCell] - g_cost_reduction
                                self.f_cost.reshape(-1)[neighbourCell] = self.f_cost.reshape(-1)[neighbourCell] - g_cost_reduction
                                self.parent.reshape(-1)[neighbourCell] = cell
                                ...
                                # Open Heap
                                self.openHeap_propagateToHead(neighbourCell)

                        else:
                            # cell is yet unknown -> initialize it and add
                            # it to the Open Heap
                            # TODO: initialize cell and add it to the Open Heap
                            self.g_cost.reshape(-1)[neighbourCell] = self.g_cost.reshape(-1)[cell] + neighbourCosts[i]
                            [y, x] = np.unravel_index(neighbourCell, self.sz)
                            delta = np.abs([x - self.x_goal, y - self.y_goal])
                            h_cost = 10 * np.abs(delta[0] - delta[1]) + 14 * np.min(delta)
                            self.f_cost.reshape(-1)[neighbourCell] = self.g_cost.reshape(-1)[neighbourCell] + h_cost
                            self.parent.reshape(-1)[neighbourCell] = cell
    ...
    def openHeap_add(self, cell):
        ...
        self.nOpenCells = self.nOpenCells + 1  #因为索引的问题，上下换了一下 ,self.nOpenCells = len(self.openHeap!=0)
    ...
    def openHeap_popHead(self):
        ...
        while True:
            ...
            if idxRight <= self.nOpenCells:
                if self.f_cost.reshape(-1)[self.openHeap[idx-1]] > self.f_cost.reshape(-1)[self.openHeap[idxLeft-1]]:
                    idx = idxLeft  # ‼️
                if self.f_cost.reshape(-1)[self.openHeap[idx - 1]] > self.f_cost.reshape(-1)[self.openHeap[idxRight-1]]:
                    idx = idxRight
